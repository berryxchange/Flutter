...
class UserPaymentMethodsPage extends StatefulWidget {
  ...
  UserPaymentMethodsPage(
      {this.thisUser,
      this.tabIndex,
      this.fromPaymentsPage,
      this.fromPaymentPageNewCard});
...
class UserPaymentMethodsPageState extends State<UserPaymentMethodsPage>
  ...
  Color paymentMethodTabColor({int index}) {
    return userPaymentMethodsBLOC.paymentMethodTabColor(
        index: index, paymentMethodIndex: paymentMethodIndex);
  }

  Color paymentMethodTabMainTextColor({int index}) {
    return userPaymentMethodsBLOC.paymentMethodTabMainTextColor(
        index: index, paymentMethodIndex: paymentMethodIndex);
  }

  Color paymentMethodTabSubtextColor({int index}) {
    return userPaymentMethodsBLOC.paymentMethodTabSubtextColor(
        index: index, paymentMethodIndex: paymentMethodIndex);
  }
...
// Getting card transactions
  showCardTransactions(CreditCardModel thisPaymentCard) async {
    ...
    if (transactionStyle == "tithe") {
      checkIfTransactionIsTithe(
          transactionStyle: transactionStyle, thisPaymentCard: thisPaymentCard);
    } else if (transactionStyle == "offering") {
      checkIfTransactionIsOffering(
          transactionStyle: transactionStyle, thisPaymentCard: thisPaymentCard);
    }
  }

  checkIfTransactionIsOffering(
      {String transactionStyle, CreditCardModel thisPaymentCard}) async {
    userPaymentMethodsBLOC
        .checkTransaction(
            transactionStyle: transactionStyle,
            transactionList: offeringTransactions,
            thisCard: thisPaymentCard)
  ...
  checkTransactionTotalCost({int index}) {
    if (transactions != null && transactions.isNotEmpty) {
      return Text(
        "\$${transactions[index].orderTotalCost / 100}",
        style: TextStyle(fontSize: 20, color: Colors.blue),
      );
    }
  }
  ...
  void handleSubmit(
      {bool isFromPaymentsPage, bool isFromPaymentPageNewCard}) async {
    if (isFromPaymentsPage == true) {
      ...
      if (form.validate()) {
        ...
        await StripePaymentService.addPaymentMethodToCustomer(
                thisUser: thisUser, thisCard: thisCard)
            .then((paymentMethodId) {
          //then sendNewCardData to FB
          print("Addable Payment Method Id: $paymentMethodId");
          //change card number
          var lastFourDigits =
              thisCard.cardNumber.substring(thisCard.cardNumber.length - 4);
  ...
  void initState() {
    ...
    userCardTitheTransactionsRef =
        userPaymentMethodsBLOC.getUserCardTitheTransactionsRef(thisUser);

    userCardOfferingTransactionsRef =
        userPaymentMethodsBLOC.getUserCardOfferingTransactionsRef(thisUser);

    //---------- Animation Setters -----------

    // set the controller from GlobalAnimations chart
    addCardAlertAnimationController = userPaymentMethodsBLOC
        .getAddCardAlertAnimationController(thisClass: this);
    ...
    addCardAlertBackgroundAnimation =
        userPaymentMethodsBLOC.getAddCardAlertBackgroundAnimation();
  ...
  setListeners() {
    ...
    userCardOfferingTransactionsRef.onChildChanged
        .listen(_onUserOfferingChanged);
    userCardOfferingTransactionsRef.onChildRemoved
        .listen(_onUserOfferingRemoved);
  }
  ...
  _getUsersOfferingEvent(Event event) {
    //thisUserInfo
    PaymentOrderModel thisOffering =
        PaymentOrderModel.fromSnapshot(event.snapshot);
        ...
        bottomNavigationBar: Container(
          ...
          child: TabBar(
            tabs: [
              Tab(
                child: Column(
                  children: [Icon(Icons.library_add_sharp), Text("Add A Card")],
                ),
              ),
              Tab(
                child: Column(
                  children: [Icon(Icons.list_alt), Text("Transactions")],
                ),
              ),
            ],
          ),
                                    ...
                                    child: OutlineButton(
                                      ...
                                      onPressed: () {
                                        handleSubmit(
                                            isFromPaymentsPage:
                                                fromPaymentsPage,
                                            isFromPaymentPageNewCard:
                                                fromPaymentPageNewCard);
                                      },
                                      child: Container(
                                        height: 50,
                                        child: Center(
